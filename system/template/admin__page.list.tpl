{block name="inner_body"}
	<form method="post">
		...
		<button type="button" onclick="window.open('?R[Page]=admin__page.edit&R[ModuleId]=papp/page&D[PAGE][D][{hash("crc32b", time())}][Active]=1','_self');" class="btn btn-primary btn-sm">Neu</button>
	<table class="table">
		<thead>
			<tr>
			<th scope="col">#</th>
		...
		{foreach from=$D.PAGE.D key="kPAG" item="PAG"}
			<tr>
				...
				<td><button type="button" onclick="window.open('?R[Page]=admin__page.edit&R[ModuleId]=papp/page&R[Id]={$kPAG}','_self');" class="btn btn-primary btn-sm">Edit</button></td>
			</tr>
		{/foreach}
		</tbody>
	</table>
